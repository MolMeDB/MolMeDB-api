...
function createMembranesRmAdmin(array $permissions = []): User
{
 ...
 */
function createMembranesRmDatasetDependencies(): array
{
...
function createMembranesRmDataset(User $owner, array $attributes = []): Dataset
{
    $dependencies = createMembranesRmDatasetDependencies();

    return Dataset::query()->create([
        'type' => Dataset::TYPE_PASSIVE,
        'name' => 'Reference dataset',
        'comment' => 'Dataset used for membranes relation manager tests.',
...
function createMembranesRmPublication(array $attributes = []): Publication
{
...
test('membranes relation manager shows membranes attached to publication only', function () {
    $user = createMembranesRmAdmin([
        PermissionEnums::PUBLICATION_VIEW->value,
        PermissionEnums::PUBLICATION_EDIT->value,
        PermissionEnums::MEMBRANE_METHOD_VIEW->value,
    ]);

    $publication = createMembranesRmPublication();
    $relatedDataset = createMembranesRmDataset($user, [
        'name' => 'Related membrane dataset',
    ]);
    $foreignDataset = createMembranesRmDataset($user, [
        'name' => 'Foreign membrane dataset',
    ]);

    $publication->membranes()->syncWithPivotValues([$relatedDataset->membrane->id], ['model_type' => Membrane::class]);

    $this->actingAs($user);

    Livewire::test(MembranesRelationManager::class, [
        'ownerRecord' => $publication,
        'pageClass' => EditPublication::class,
    ])
        ->assertOk()
        ->assertCanSeeTableRecords([$relatedDataset->membrane])
        ->assertCanNotSeeTableRecords([$foreignDataset->membrane]);
});
