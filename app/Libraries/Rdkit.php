...
class Rdkit extends IdentifiersWorker
{
    ...
    */
    ...
    public function canonize_smiles($smiles, $throwable = false)
    {
        ...
        try
        {
            $response = Http::timeout(30)->acceptJson()->withUrlParameters(self::$url_parameters)->get('{+host}/smiles/canonize', $params);
            $body = json_decode($response->body());
            if($response->successful() && count($body) == 1)
            {
                // Protect links
                $smiles = preg_replace('/\[\*:([0-9]+)\]/', '[$1:]', $body[0]);
                return $smiles;
            }
    ...
    public function get_general_info($smiles)
    {
        ...
        try
        {
            $response = Http::timeout(30)->acceptJson()->withUrlParameters(self::$url_parameters)->get('{+host}/general', $params);
            $body = json_decode($response->body());
