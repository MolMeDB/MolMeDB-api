...
class CreateSubstance extends CreateRecord
{
    ...
    protected function afterCreate(array $data): void
    {
        ...
        $substance->identifiers()->create([
            'value' => $data['smiles'],
            'type'  => SubstanceIdentifier::TYPE_SMILES,
            'user_id' => Auth::user()->id,
            'is_active' => true,
            'state' => SubstanceIdentifier::STATE_VALIDATED
        ]);

        $substance->identifiers()->create([
            'value' => $data['inchikey'],
            'type'  => SubstanceIdentifier::TYPE_INCHIKEY,
            'user_id' => Auth::user()->id,
            'is_active' => true,
            'state' => SubstanceIdentifier::STATE_VALIDATED,
            'server' => SubstanceIdentifier::SERVER_RDKIT
        ]);
    }
