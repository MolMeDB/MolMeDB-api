...
export default function SectionMenu() {
  ...
  return (
    <div className="flex flex-col w-full h-full sticky">
      {menuItems.map((item) => (
        <Group key={item.id} title={item.title} children={item.children} />
      ))}
    </div>
  );
}
...
function Group(props: { title: string; children?: MenuItem[] }) {
  return (
    <div className="flex flex-col gap-3 bg-zinc-100 border-t-1 border-white p-4">
      <Link
        href={"/docs/about"}
      ...
      {props.children?.map((item) => (
        <Link
          key={item.id}
          href={"docs/inner"}
          className="pl-4 text-sm hover:text-primary-400 transition-colors duration-400 ease-in-out"
        >
          {item.title}
        </Link>
      ))}
